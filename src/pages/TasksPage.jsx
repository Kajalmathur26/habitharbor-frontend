      ...
      <div className="space-y-8">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <TaskColumn id="pending" label="To Do" count={groupedTasks.pending.length} color="bg-amber-400" tasks={groupedTasks.pending} onToggle={toggleComplete} onEdit={startEdit} onDelete={deleteTask} onAdd={() => setShowForm(true)} />
            <TaskColumn id="in_progress" label="In Progress" count={groupedTasks.in_progress.length} color="bg-blue-400 border-2 border-primary/20" tasks={groupedTasks.in_progress} onToggle={toggleComplete} onEdit={startEdit} onDelete={deleteTask} onAdd={() => setShowForm(true)} />
            <TaskColumn id="completed" label="Completed" count={groupedTasks.completed.length} color="bg-emerald-400" tasks={groupedTasks.completed} onToggle={toggleComplete} onEdit={startEdit} onDelete={deleteTask} onAdd={() => setShowForm(true)} />
          </div>
        )}
      </div>
...
function TaskColumn({ id, label, count, color, tasks, onToggle, onEdit, onDelete, onAdd }) {
  return (
        ...
        <div className="space-y-4 flex-1 p-3 rounded-3xl bg-secondary/40 border-2 border-border/30 group-hover/col:border-primary/20 transition-all bg-gradient-to-b from-secondary/20 to-secondary/5 min-h-[500px] shadow-inner">
          ...
          {tasks.length === 0 && (
            <div className="h-64 flex flex-col items-center justify-center p-6 rounded-2xl border-2 border-dashed border-border/60 bg-secondary/30 mt-10">
              <button
                onClick={onAdd}
                className="w-16 h-16 rounded-full border-2 border-dashed border-primary/60 bg-primary/10 flex items-center justify-center hover:bg-primary/20 hover:border-primary/80 transition-all group"
              >
